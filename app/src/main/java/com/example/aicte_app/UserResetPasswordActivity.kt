...
class UserResetPasswordActivity : AppCompatActivity() {
    ...
    private fun verifyUser()
    {
                ...
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        ...
                        if (snapshot.exists())
                        {
                            ...
                            if (ans1 != answer1) {
                                Toast.makeText(this@UserResetPasswordActivity, "Your First Answer is wrong.", Toast.LENGTH_LONG).show()
                            } else if (ans2 != answer2) {
                                Toast.makeText(this@UserResetPasswordActivity, "Your Second Answer is wrong.", Toast.LENGTH_LONG).show()
                            } else {

                                val builder = AlertDialog.Builder(this@UserResetPasswordActivity)
                                builder.setTitle("New Password")

                                val newPassword: EditText = EditText(this@UserResetPasswordActivity)
                                    ...
                                    if (newPassword.text.toString() != null) {
                                        ref.child("password").setValue(newPassword.text.toString())
                                            .addOnCompleteListener {
                                                if (it.isSuccessful) {
                                                    Toast.makeText(this@UserResetPasswordActivity, "Password Changed Successfully", Toast.LENGTH_LONG).show()
                                                    val intent = Intent(this@UserResetPasswordActivity, UserLogInActivity::class.java)
                            ...
                            Toast.makeText(this@UserResetPasswordActivity, "You have not set the security questions", Toast.LENGTH_LONG).show()
                        }

                    }
                    else
                    {
                        Toast.makeText(this@UserResetPasswordActivity, "This phone number not exist", Toast.LENGTH_LONG).show()
                    }
        ...
        {
            Toast.makeText(this@UserResetPasswordActivity, "Please complete the form", Toast.LENGTH_LONG).show()
        }
    ...
    private fun setAnswer()
    {
        ...
        if(answer1=="" && answer2=="")
        {
            Toast.makeText(this@UserResetPasswordActivity,"Please Answer Both the questions...",Toast.LENGTH_LONG).show()
        }
        else
        {
            ...
            ref.child("Security Questions").updateChildren(userDataMap).addOnCompleteListener{
                if(it.isSuccessful)
                {
                    Toast.makeText(this@UserResetPasswordActivity,"You have answered the security Questions Successfully.",Toast.LENGTH_LONG).show()
                    val intent = Intent(this@UserResetPasswordActivity, HomeActivity::class.java)
                    startActivity(intent)
                    finish()
                }
            }
